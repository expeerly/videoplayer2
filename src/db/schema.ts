...
export const processedVideos = pgTable('processed_videos', {
  id: serial('id').primaryKey(),
  gtinEan: text('gtin_ean').notNull(),
  gtinEanVariants: jsonb('gtin_ean_variants').notNull(),
  muxPlaybackId: text('mux_playback_id').notNull(),
  productName: text('product_name').notNull(),
  rating: real('rating'),
  reviewerFirstName: text('reviewer_first_name'),
  reviewerLastName: text('reviewer_last_name'),
  reviewerProfilePic: text('reviewer_profile_pic'),
  status: text('status'),
  upc: text('upc'),
  upcVariants: jsonb('upc_variants'),
  createdAt: timestamp('created_at', {
    precision: 6,
    withTimezone: true,
  }).defaultNow(),
